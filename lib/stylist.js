...
export function suggestProps({ tone, description = "", limit = 6 }) {
  const hints = TONE_HINTS[tone] || [];
  const words = (description.toLowerCase().match(/[a-z]{4,}/g) || []).filter(
    (w) => !["with", "that", "this", "room", "have", "need", "want", "space", "some", "look"].includes(w)
  );
...
export function stylistIntro({ tone, description, count }) {
  const label = (MOOD_TONES.find((t) => t.key === tone) || {}).label || "that";
  const desc = description ? ` around "${description.trim().slice(0, 70)}"` : "";
  return `For a ${label.toLowerCase()} space${desc}, here are ${count} props from partner stores that would fit. Add any to your board.`;
}
