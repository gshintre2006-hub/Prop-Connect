...
/* Vendor Portal stores aren't in the static array above — the client-side
   catalog fetch in app/providers.jsx registers them here once loaded, so
   storeById() (used by e.g. the cart/moodboard PDF exporters) resolves both
   the static network and live vendor stores without every caller having to
   know the difference. */
let _vendorStoreIndex = {};
export function _registerVendorStores(stores) {
  _vendorStoreIndex = Object.fromEntries(stores.map((s) => [s.id, s]));
}
export const storeById = (id) => STORES.find((s) => s.id === id) || _vendorStoreIndex[id];
